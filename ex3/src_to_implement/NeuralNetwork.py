...
class NeuralNetwork:
    ...
    def phase(self, phase_val):
        self.p_phase = phase_val

        """Set the phase of the network (all layers) to training or testing."""
       
        for layer in self.layers:
            layer.testing_phase = phase_val
    ...
    def forward(self):
        ...
        # If a regularizer is set, add its norm to the loss
        if self.regularizer is not None:
            regularizer_loss = 0
            for layer in self.layers:
                if hasattr(layer, "weights"):
                    regularizer_loss += self.regularizer.norm(layer.weights)
            loss_value += regularizer_loss
    ...
    def train(self, iterations):
        ...
        """
        self.phase = False  # Set the network to training phase
    ...
    def test(self, input_tensor):
        ...
        self.phase = True  # Set the network to testing phase
        for layer in self.layers:
